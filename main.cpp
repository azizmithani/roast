...
void help() {
    ...
    cerr << "   --inner_itr <INT> \t\t\t Number of Inner iterations threshold, def 30" << endl;

    cerr << "   --outer_itr <INT> \t\t\t Number of Outer Iterations threshold, def 100" << endl;

    cerr << "   --improvment_TH <INT> \t\t keep improving until number of improved contigs meet threshold, def 1" << endl;

    cerr << "   --min_extended_contigs <INT> \t keep extending until number of extended contigs meet threshold, def 1" << endl;

    cerr << "   --max_memory_TRINITY <INT> \t\t maximum memory for TRINITY, def 20" << endl;

    cerr << "   --threads <INT> \t\t\t Number of threads, deft 8" << endl;

    cerr << "   --threadsForSamSort <INT> \t\t Number of threads for samtools sort, def 2" << endl;

    cerr << "   --memForSamSort <INT> \t\t Memory for samtools sort, default 768M" << endl;

    cerr << "   --complete_cleanup \t\t\t Complete cleanup of intermediate files, def partial cleanup" << endl;

    cerr << "   --nochange_header \t\t\t Don't change header of final improved assembly file, def change header" << endl;

    cerr << "   --cdHitEST <INT> \t\t\t 0 for no CD-HIT-EST, 1 for CD-HIT-EST only in the start, 2 for CD-HIT-EST after every iteration, def 1" << endl;

    cerr << "   --mapping_quality \t\t\t Minimum mapping quality to filter Bam file, default 20" << endl << endl;

    cerr << "   --sc_support_TH \t\t\t Softclips support for a position for Incomplete and fragmented contigs and false chimera process, def 0.75" << endl;

    cerr << "   --min_SCs_reads <INT> \t\t Minimum number of softclips reads for extension of Incomplete contigs, def 3" << endl;

    cerr << "   --sc_cons_len_TH <INT> \t\t Minimum length of consensus sequence generated by softclips to BLAST for fragmented contigs Identification, def 10" << endl;

    cerr << "   --discard_contig_corner_len_TH <INT>  Minimum number of bases allowed to discard from the corner of contigs while finding overlapped edges, def 10" << endl;

    cerr << "   --max_allowed_gaps <INT> \t\t Maximum gaps allowed to find overlaps between the corners of the two contigs, def 0" << endl;

    cerr << "   --edge_boundary <INT> \t\t Terminal region for mapped reads to be considered for un-mapped and distantly mapped reads, def 2x of read length" << endl;

    cerr << "   --min_unmapped_reads <INT> \t\t Minimum number of unmapped reads to generate CAP3 assembly for incomplete contigs extension, def 5" << endl;

    cerr << "   --min_distMapped_reads <INT> \t Minimum number of distantly mapped reads to consider as read island for merging of fragmented contigs, def 3" << endl;

    cerr << "   --contig_boundary <true> \t\t Check overlap between two fragmented contigs within 5% read length of the contig boundary, def read island boundary" << endl;

    cerr << "   --min_allowed_unmapped_ext <INT> \t Minimum number of bases to be considered for valid extension using unmapped reads, def 50% of read length" << endl;

    cerr << "   --one_side_allowed_SCs_RI <INT> \t Maximum % of softclips of the total read length allowed at one side of read to consider it for Read Island, def 25" << endl;

    cerr << "   --each_side_allowed_SCs_RI <INT> \t Maximum % of softclips of the total read length allowed at both sides of read to consider it for Read Island, def 12" << endl;

    cerr << "   --sc_start_pos_from_terminus <INT> \t For terminal softclips extraction, define starting position to consider, def 25" << endl;

    cerr << "   --min_overlap_TH <INT> \t\t Minimum length of overlapped sequence to consider for merging fragmented contigs and CAP3 assemblies, def 20 bases" << endl;
    ...
    cerr << "   --coverage_drop_TH <INT> \t\t Maximum coverage ratio between two consecutive positions to process for false chimera identification process, def 0.2" << endl;

    cerr << "   --st_end_boundary <INT> \t\t Consider coverage change within start and end boundary of the contig, def one and half of read length " << endl;

    cerr << "   --blast_score_TH <INT> \t\t BLAST hit identity and coverage score for mis-assembly/false chimera, def 90" << endl;

    cerr << "   --ignore_short_seq <INT> \t\t Minimum length of the contig to remove from the assembly, def 200" << endl;

    cerr << "   --Insert_Ns <INT> \t\t\t Number of Ns to insert between contig and its CAP3 assembly in the absence of overlap, def 5" << endl << endl;
  ...
  //  cerr << "   --transRate_score \t\t\t set environment variable TransRate_Home to point TransRate installation directory to calculate TransRate score of final improved assembly, default no score" << endl;
